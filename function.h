...
void Acceleration(int i) {
	if (moveFlag) {
		...
		stoneInfo[i].x = 660.0 + sin(Rad(course * 2.5)) * nowStonePos;
		stoneInfo[i].y = 100.0 + cos(Rad(course * 2.5)) * nowStonePos;
	}
...
void DrawInfo() {
	DrawFormatStringToHandle(0, 20, 0x000000, Cica16, "course:%d,power:%d\nspeed:%f,nowStonePos:%f\nsetFlag:%s,moveFlag:%s", course, power, stoneInfo[0].speed, nowStonePos, setFlag?"true":"false", moveFlag?"true":"false");
}
